...
class ContrastiveNeuralUCB(UCB):
    """Contrastive Neural UCB.
    """
    ...
    def __init__(self,
                 ...
                 train_every=10,
                 throttle=1,
                 triplet_weighting_term=.1,
                 triplet_alpha=.1,
                 use_cuda=False,
                 ):
        self.bandit_contrast_pos = bandit_contrast_pos
        self.bandit_contrast_neg = bandit_contrast_neg
        # hidden size of the NN layers
        self.hidden_size = hidden_size
        # number of layers
        self.n_layers = n_layers
        self.triplet_weighting_term = triplet_weighting_term
    ...
    def train(self):
        ...
        x_train_pos = torch.FloatTensor(self.bandit_contrast_pos.features[iterations_so_far, actions_so_far]).to(
            self.device)
        x_train_neg = torch.FloatTensor(self.bandit_contrast_neg.features[iterations_so_far, actions_so_far]).to(
            self.device)

        y_train = torch.FloatTensor(self.bandit.rewards[iterations_so_far, actions_so_far]).squeeze().to(self.device)
        y_train_pos = torch.FloatTensor(
            self.bandit_contrast_pos.rewards[iterations_so_far, actions_so_far]).squeeze().to(self.device)
        y_train_neg = torch.FloatTensor(
            self.bandit_contrast_neg.rewards[iterations_so_far, actions_so_far]).squeeze().to(self.device)

        # train mode
        self.model.train()
        for _ in range(self.epochs):
            y_pred = self.model.forward(x_train).squeeze()
            y_pred_pos = self.model.forward(x_train_pos).squeeze()
            y_pred_neg = self.model.forward(x_train_neg).squeeze()
            # Reformulate into Triplet Loss
            loss = nn.MSELoss()(y_train,y_pred) + self.triplet_weighting_term*(nn.MSELoss()(y_pred,y_pred_pos) - nn.MSELoss()(y_pred,y_pred_neg)+ self.triplet_alpha)
            # Original Loss
